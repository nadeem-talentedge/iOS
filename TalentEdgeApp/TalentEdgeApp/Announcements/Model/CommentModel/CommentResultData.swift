...
public final class CommentResultData: NSCoding {
  ...
  public required init(json: JSON) {
    if let items = json[SerializationKeys.reply].array { reply = items.map { Reply(json: $0) } }
  ...
  required public init(coder aDecoder: NSCoder) {
    self.reply = aDecoder.decodeObject(forKey: SerializationKeys.reply) as? [Reply]
